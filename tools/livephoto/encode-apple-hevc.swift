...
enum EncodeError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .usage:
            return "Usage: swift encode-apple-hevc.swift <input.mov> <output.mov> [bitrate] [keyframe-interval]"
        case .missingVideoTrack:
...
guard (3...5).contains(arguments.count) else { throw EncodeError.usage }
